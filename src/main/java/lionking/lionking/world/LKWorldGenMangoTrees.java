...
public class LKWorldGenMangoTrees extends WorldGenerator {
    ...
    public LKWorldGenMangoTrees(boolean notify) {
        super(notify);
        FMLLog.fine("Initialized LKWorldGenMangoTrees with notify=%b", notify);
    }

    public boolean generate(World world, Random random, int x, int y, int z) {
        FMLLog.fine("Attempting to generate mango tree at (%d, %d, %d)", x, y, z);

        if (mod_LionKing.prideWood == null || mod_LionKing.mangoLeaves == null) {
            FMLLog.severe("Cannot generate mango tree at (%d, %d, %d): prideWood or mangoLeaves is null", x, y, z);
            return false;
        }

        int height = MIN_HEIGHT + random.nextInt(HEIGHT_VARIATION);
        if (y < 1 || y + height + 1 >= MAX_WORLD_HEIGHT) {
            FMLLog.fine("Cannot generate mango tree at (%d, %d, %d): invalid height (height=%d)", x, y, z, height);
            return false;
        }

        if (!checkSpace(world, x, y, z, height)) {
            FMLLog.fine("Cannot generate mango tree at (%d, %d, %d): insufficient space", x, y, z);
            return false;
        }

        Block groundBlock = world.getBlock(x, y - 1, z);
        if (groundBlock != Blocks.grass && groundBlock != Blocks.dirt || y >= MAX_WORLD_HEIGHT - height - 1) {
            FMLLog.fine("Cannot generate mango tree at (%d, %d, %d): invalid ground block %s", x, y, z, groundBlock != null ? groundBlock.getUnlocalizedName() : "null");
            return false;
        }
        ...
        FMLLog.info("Successfully generated mango tree at (%d, %d, %d) with height=%d", x, y, z, height);
        return true;
    }
            ...
            for (int dx = x - radius; dx <= x + radius; dx++) {
                ...
                for (int dz = z - radius; dz <= z + radius; dz++) {
                    ...
                    if ((Math.abs(distX) != radius || Math.abs(distZ) != radius || 
                        ...
                        world.setBlock(dx, dy, dz, mod_LionKing.mangoLeaves, 0, 2);
                        FMLLog.fine("Placed mangoLeaves at (%d, %d, %d)", dx, dy, dz);
                    }
                }
            }
        }
    ...
    private void generateTrunk(World world, int x, int y, int z, int height) {
        for (int dy = 0; dy < height; dy++) {
            ...
            if (isBlockReplaceable(world, block, x, y + dy, z)) {
                world.setBlock(x, y + dy, z, mod_LionKing.prideWood, 2, 2);
                FMLLog.fine("Placed prideWood at (%d, %d, %d), meta=2", x, y + dy, z);
            }
        }
    }

    private boolean isBlockReplaceable(World world, Block block, int x, int y, int z) {
        return block == null || block.getMaterial() == Material.air || block.isLeaves(world, x, y, z) || block == mod_LionKing.mangoLeaves;
    }
}