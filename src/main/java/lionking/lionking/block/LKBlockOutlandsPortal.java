...
import lionking.common.LKMaterialPortal;
...
public class LKBlockOutlandsPortal extends BlockBreakable {
    public LKBlockOutlandsPortal() {
        super("lionking:outlandsPortal", LKMaterialPortal.PORTAL_MATERIAL, false); 
        setCreativeTab(null);
        setBlockName("outlandsPortal");
    }
    ...
    public static boolean tryToCreatePortal(World world, int x, int y, int z) {
        int axisX = 0;
        int axisZ = 0;
        if (world.getBlock(x - 1, y, z) == mod_LionKing.outlandsPortalFrame || world.getBlock(x + 1, y, z) == mod_LionKing.outlandsPortalFrame) {
            axisX = 1;
        }
        if (world.getBlock(x, y, z - 1) == mod_LionKing.outlandsPortalFrame || world.getBlock(x, y, z + 1) == mod_LionKing.outlandsPortalFrame) {
            axisZ = 1;
        }
        ...
        for (int offsetX = -1; offsetX <= 2; offsetX++) {
            for (int offsetY = -1; offsetY <= 3; offsetY++) {
                ...
                Block block = world.getBlock(x + axisX * offsetX, y + offsetY, z + axisZ * offsetX);
                if (isEdge) {
                    if (block != mod_LionKing.outlandsPortalFrame) return false;
                } else if (!world.isAirBlock(x + axisX * offsetX, y + offsetY, z + axisZ * offsetX)) { 
                    return false;
                }
            }
        }
        ...
        for (int width = 0; width < 2; width++) {
            for (int height = 0; height < 3; height++) {
                world.setBlock(x + axisX * width, y + height, z + axisZ * width, mod_LionKing.outlandsPortal, 0, 2);
            }
        }
        return true;
    }
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block neighborBlock) {
        ...
        if (world.getBlock(x, bottomY - 1, z) != mod_LionKing.outlandsPortalFrame) {
        ...
        if (height != 3 || world.getBlock(x, bottomY + height, z) != mod_LionKing.outlandsPortalFrame) {
            world.setBlockToAir(x, y, z);
            return;
        }
        ...
        if ((world.getBlock(x + axisX, y, z + axisZ) != mod_LionKing.outlandsPortalFrame || world.getBlock(x - axisX, y, z - axisZ) != this) &&
            (world.getBlock(x - axisX, y, z - axisZ) != mod_LionKing.outlandsPortalFrame || world.getBlock(x + axisX, y, z + axisZ) != this)) {
            world.setBlockToAir(x, y, z);
        }
    }
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        if (entity.ridingEntity == null && entity.riddenByEntity == null && entity instanceof EntityPlayer) {
            ...
            mod_LionKing.proxy.setInOutlandsPortal(player);
        }
    }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random) {
        if (random.nextInt(100) == 0) {
            world.playSoundEffect(x + 0.5D, y + 0.5D, z + 0.5D, "portal.portal", 1.0F, random.nextFloat() * 0.4F + 0.8F);
        }
        for (int i = 0; i < 4; i++) {
